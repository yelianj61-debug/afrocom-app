<?php /* RIVO — index.php */ ?><!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8"/>
...
<div id="page-dashboard" class="page">
  ...
  <div style="padding:1.5rem;max-width:1100px;padding-bottom:80px">
      ...
      <div id="tab-bord" class="tab-panel active fade-in">
        <div style="border-radius:1.5rem;padding:1.5rem;color:white;margin-bottom:1.5rem;background:linear-gradient(135deg,#2563EB,#1D4ED8)">
          <h1 id="welcome-h" style="font-size:1.4rem;font-weight:900;margin-bottom:.3rem">Bienvenue !!!</h1>
          <div id="user-rivo-id" style="font-size:.78rem;color:#bfdbfe;font-weight:700;margin-bottom:1rem;letter-spacing:.03em">🆔 R.I.V.O_...</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:.625rem">
            <div style="background:rgba(255,255,255,.15);border-radius:.875rem;padding:.875rem">
              <div style="font-size:.65rem;color:#bfdbfe;font-weight:700;margin-bottom:.2rem;text-transform:uppercase;letter-spacing:.04em">💰 Solde principal</div>
              <div id="bal-principal" style="font-size:1.2rem;font-weight:900;margin-bottom:.5rem">0 FCFA</div>
              <button onclick="openRecharge()" style="background:rgba(255,255,255,.25);border:none;color:white;font-size:.7rem;font-weight:700;padding:.3rem .55rem;border-radius:.45rem;cursor:pointer" onmouseover="this.style.background='rgba(255,255,255,.38)'" onmouseout="this.style.background='rgba(255,255,255,.25)'">➕ Recharger</button>
            </div>
            <div style="background:rgba(255,255,255,.15);border-radius:.875rem;padding:.875rem">
              <div style="font-size:.65rem;color:#bfdbfe;font-weight:700;margin-bottom:.2rem;text-transform:uppercase;letter-spacing:.04em">🤝 Solde parrainage</div>
              <div id="bal-parrainage" style="font-size:1.2rem;font-weight:900;margin-bottom:.5rem">0 FCFA</div>
      ...
      <div id="tab-profil" class="tab-panel fade-in">
        ...
        <!-- Retrait accessible depuis le tableau de bord (bouton 💸 Retirer) -->

        <!-- Historique des transactions -->
        <div class="card" style="padding:1.25rem;margin-bottom:1.5rem" id="hist-section">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem">
            <h3 style="font-weight:800;font-size:1.05rem;color:#1F2937">📋 Historique</h3>
            <div style="display:flex;gap:.4rem">
              <button id="hist-tab-achats" onclick="loadHistorique('achats')" style="font-size:.75rem;font-weight:700;padding:.3rem .75rem;border-radius:9999px;border:none;background:#2563EB;color:white;cursor:pointer">💳 Achats</button>
              <button id="hist-tab-recharges" onclick="loadHistorique('recharges')" style="font-size:.75rem;font-weight:700;padding:.3rem .75rem;border-radius:9999px;border:1.5px solid #e5e7eb;background:none;color:#6b7280;cursor:pointer">➕ Recharges</button>
              <button id="hist-tab-retraits" onclick="loadHistorique('retraits')" style="font-size:.75rem;font-weight:700;padding:.3rem .75rem;border-radius:9999px;border:1.5px solid #e5e7eb;background:none;color:#6b7280;cursor:pointer">💸 Retraits</button>
            </div>
          </div>
          <div id="hist-content"><p style="color:#9ca3af;text-align:center;padding:1.5rem">Chargement…</p></div>
        </div>
      ...
      <div id="tab-recompenses" class="tab-panel fade-in">
        ...
        <div style="display:flex;border-bottom:2px solid #f1f5f9;margin-bottom:1.25rem;overflow-x:auto">
          ...
          <button id="rt-partners" onclick="loadRecompensesTab('partners')" style="flex:1;padding:.875rem .5rem;background:none;border:none;border-bottom:3px solid transparent;cursor:pointer;font-weight:500;font-size:.875rem;color:#6b7280;white-space:nowrap">🤝 Partenaires</button>
        </div>
  ...
  <div id="pay-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:100;align-items:center;justify-content:center;padding:1rem">
    <div style="background:white;border-radius:1.5rem;padding:1.75rem;max-width:380px;width:100%;box-shadow:0 25px 60px rgba(0,0,0,.3)">
      <div style="text-align:center;margin-bottom:1.25rem">
        <div style="width:3.5rem;height:3.5rem;background:#dbeafe;border-radius:9999px;display:flex;align-items:center;justify-content:center;font-size:1.75rem;margin:0 auto .75rem">🛒</div>
        <h3 style="font-size:1.15rem;font-weight:800;color:#1F2937;margin-bottom:.3rem">Acheter cette formation</h3>
        <p id="pm-title" style="color:#6b7280;font-size:.875rem;margin-bottom:.25rem"></p>
        <p id="pm-price" style="font-size:1.6rem;font-weight:900;color:#2563EB"></p>
      </div>
      <p style="font-size:.875rem;font-weight:700;color:#374151;margin-bottom:.75rem">Quel solde utiliser ?</p>
      <div style="display:flex;flex-direction:column;gap:.5rem;margin-bottom:1.25rem">
        <label id="pm-opt-principal" style="display:flex;align-items:center;gap:.75rem;padding:.75rem 1rem;border:2px solid #2563EB;border-radius:.875rem;cursor:pointer;background:#eff6ff">
          <input type="radio" name="pay-source" value="principal" checked style="accent-color:#2563EB"/>
          <span><span style="font-weight:700;color:#1e40af">💰 Solde principal</span><br><span id="pm-bal-principal" style="font-size:.8rem;color:#6b7280">0 FCFA</span></span>
        </label>
        <label id="pm-opt-parrainage" style="display:flex;align-items:center;gap:.75rem;padding:.75rem 1rem;border:2px solid #e5e7eb;border-radius:.875rem;cursor:pointer">
          <input type="radio" name="pay-source" value="parrainage" style="accent-color:#2563EB"/>
          <span><span style="font-weight:700;color:#374151">🤝 Solde parrainage</span><br><span id="pm-bal-parrainage" style="font-size:.8rem;color:#6b7280">0 FCFA</span></span>
        </label>
      </div>
      <div style="display:flex;gap:.625rem">
        <button onclick="closePayModal()" style="flex:1;padding:.7rem;border-radius:.875rem;border:2px solid #e5e7eb;background:none;color:#6b7280;font-weight:700;cursor:pointer">Annuler</button>
        <button onclick="confirmPay()" id="pay-btn" class="btn-accent" style="flex:1;justify-content:center;padding:.7rem">⚡ Payer</button>
      </div>
    </div>
  </div>

  <!-- Modal Rechargement (3 étapes) -->
  <div id="modal-recharge" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:200;align-items:center;justify-content:center;padding:1rem">
    <div style="background:white;border-radius:1.5rem;padding:1.75rem;max-width:400px;width:100%;box-shadow:0 25px 60px rgba(0,0,0,.3);max-height:90vh;overflow-y:auto">
      <!-- Étape 1 : formulaire -->
      <div id="rch-step1">
        <h3 style="font-size:1.15rem;font-weight:800;color:#1F2937;margin-bottom:1.25rem">➕ Recharger le solde principal</h3>
        <div style="display:flex;flex-direction:column;gap:.875rem">
          <div>
            <label style="display:block;font-size:.875rem;font-weight:700;color:#374151;margin-bottom:.35rem">Votre ID RIVO</label>
            <input type="text" id="rch-id" class="input" readonly style="background:#f9fafb;color:#6b7280"/>
          </div>
          <div>
            <label style="display:block;font-size:.875rem;font-weight:700;color:#374151;margin-bottom:.35rem">Montant (FCFA)</label>
        ...
        <div style="display:flex;gap:.625rem;margin-top:1.25rem">
          <button onclick="closeRecharge()" style="flex:1;padding:.7rem;border-radius:.875rem;border:2px solid #e5e7eb;background:none;color:#6b7280;font-weight:700;cursor:pointer">Annuler</button>
          <button onclick="doRechargeStep2()" class="btn-primary" style="flex:1;justify-content:center;padding:.7rem">Continuer →</button>
        </div>
      </div>
      <!-- Étape 2 : confirmation -->
      <div id="rch-step2" style="display:none;text-align:center">
        <div style="font-size:3rem;margin-bottom:.75rem">💳</div>
        <h3 style="font-size:1.1rem;font-weight:800;color:#1F2937;margin-bottom:.75rem">Confirmer le rechargement</h3>
        <p id="rch-confirm-msg" style="color:#374151;font-size:.95rem;margin-bottom:1.5rem;line-height:1.5"></p>
        <div style="display:flex;gap:.625rem">
          <button onclick="goRchStep(1)" style="flex:1;padding:.7rem;border-radius:.875rem;border:2px solid #e5e7eb;background:none;color:#6b7280;font-weight:700;cursor:pointer">Non</button>
          <button onclick="doRechargeConfirm()" id="rch-confirm-btn" class="btn-primary" style="flex:1;justify-content:center;padding:.7rem">Oui, confirmer</button>
        </div>
      </div>
      <!-- Étape 3 : succès -->
      <div id="rch-step3" style="display:none;text-align:center">
        <div style="font-size:3.5rem;margin-bottom:.75rem">✅</div>
        <h3 style="font-size:1.1rem;font-weight:800;color:#10b981;margin-bottom:.75rem">Demande envoyée !</h3>
        <p style="color:#374151;font-size:.9rem;line-height:1.6;margin-bottom:.75rem">Votre demande de rechargement a été transmise.<br>Un assistant va valider sous <strong>2 à 5 minutes</strong>.</p>
        <div style="background:#fef9c3;border:1px solid #fbbf24;border-radius:.75rem;padding:.75rem 1rem;margin-bottom:1.5rem;text-align:left">
          <p style="font-size:.82rem;color:#92400e;font-weight:700;margin-bottom:.2rem">📱 Important</p>
          <p style="font-size:.82rem;color:#92400e;line-height:1.5">Veuillez garder votre téléphone allumé et accessible. L'assistant vous contactera pour valider le paiement.</p>
        </div>
        <button onclick="closeRecharge()" class="btn-primary" style="width:100%;justify-content:center;padding:.75rem">← Retour</button>
      </div>
    </div>
  </div>

  <!-- Modal Retrait (solde parrainage) -->
    ...
    <div style="background:white;border-radius:1.5rem;padding:1.75rem;max-width:400px;width:100%;box-shadow:0 25px 60px rgba(0,0,0,.3);max-height:90vh;overflow-y:auto">
      ...
      <div style="display:flex;flex-direction:column;gap:.875rem;margin-bottom:1.25rem">
        ...
        <div id="wd-section-card" style="display:none">
          ...
          <div style="margin-bottom:.75rem"><label style="display:block;font-size:.875rem;font-weight:700;color:#374151;margin-bottom:.35rem">IBAN</label><input type="text" id="wd-iban" class="input" placeholder="FR76 3000…"/></div>
        ...
        <div id="wd-section-other" style="display:none">
          <div style="margin-bottom:.75rem"><label style="display:block;font-size:.875rem;font-weight:700;color:#374151;margin-bottom:.35rem">Moyen</label><input type="text" id="wd-other-method" class="input" placeholder="PayPal, Western Union…"/></div>
          <div><label style="display:block;font-size:.875rem;font-weight:700;color:#374151;margin-bottom:.35rem">Identifiant</label><input type="text" id="wd-other-detail" class="input" placeholder="Email, numéro…"/></div>
        </div>
      </div>
...
class _ProxyQuery{
  constructor(t){this._q={type:'query',table:t,action:null,data:null,filters:[],order:null,limit:null,single:false,select:'*'};}
  select(c){this._q.action='select';this._q.select=c;return this;}
  insert(d){this._q.action='insert';this._q.data=d;return this;}
  update(d){this._q.action='update';this._q.data=d;return this;}
  delete(){this._q.action='delete';return this;}
  eq(c,v){this._q.filters.push({op:'eq',col:c,val:v});return this;}
  neq(c,v){this._q.filters.push({op:'neq',col:c,val:v});return this;}
  gt(c,v){this._q.filters.push({op:'gt',col:c,val:v});return this;}
  gte(c,v){this._q.filters.push({op:'gte',col:c,val:v});return this;}
  lt(c,v){this._q.filters.push({op:'lt',col:c,val:v});return this;}
  lte(c,v){this._q.filters.push({op:'lte',col:c,val:v});return this;}
  in(c,v){this._q.filters.push({op:'in',col:c,val:v});return this;}
  not(c,op,v){this._q.filters.push({op:'not_'+op,col:c,val:v});return this;}
  ilike(c,v){this._q.filters.push({op:'ilike',col:c,val:v});return this;}
  is(c,v){this._q.filters.push({op:'is',col:c,val:v});return this;}
  order(c,o){this._q.order={col:c,...(o||{})};return this;}
  limit(n){this._q.limit=n;return this;}
  single(){this._q.single=true;return this;}
  then(res,rej){return _apiCall(this._q).then(res,rej);}
}
const sbAdmin={
  rpc:(fn,params)=>({then:(res,rej)=>_apiCall({type:'rpc',fn,params}).then(res,rej)}),
  from:(table)=>new _ProxyQuery(table)
};
  ...
  function tryLoad(i){
    ...
    s.onload=function(){
      try{
        ...
        // sbAdmin est le proxy api.php — pas de 2e client Supabase ici
        // ── Init Firebase Auth ────────────────────────
...
async function loadProfileData(){
  ...
  try{
    ...
    if(!p){
      ...
      const rivoIdFb='R.I.V.O_'+Math.floor(Math.random()*900000000+100000000);
      const{data:newP,error:insertFallbackErr}=await withTimeout(sbAdmin.from('profiles').insert({
        ...
        email:FBU.email,
        referral_code:refCode, rivo_id:rivoIdFb,
    ...
    if(!p.rivo_id){
      ...
      sbAdmin.from('profiles').update({rivo_id:newRivoId}).eq('id',p.id);
      p.rivo_id=newRivoId;
    }
    ...
    const soldeRef=fmt(p.referral_balance||0,p.currency||'XOF');
    const[ic,lb,bg,co]=badgeInfo(p.badge_level);
    document.getElementById('welcome-h').textContent=`Bienvenue ${fn} !!!`;
    const rivoIdEl=document.getElementById('user-rivo-id');if(rivoIdEl)rivoIdEl.textContent='🆔 '+p.rivo_id;
    const balP=document.getElementById('bal-principal');if(balP)balP.textContent=solde;
    const balR=document.getElementById('bal-parrainage');if(balR)balR.textContent=soldeRef;
    const balD=document.getElementById('bal-display');if(balD)balD.textContent=soldeRef;
...
async function doRegister(e){
  ...
  try{
    ...
    const uid=crypto.randomUUID();
    const refCode='RIVO-'+Math.random().toString(36).substring(2,8).toUpperCase();
    const rivoId='R.I.V.O_'+Math.floor(Math.random()*900000000+100000000);
  ...
  document.getElementById('bord-grid').innerHTML=cs.map(c=>{
    ...
    return`<div class="card fade-in">
      <div style="height:10rem;background:linear-gradient(135deg,#2563EB,#1D4ED8);display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden">
        ${c.cover_url?`<img src="${c.cover_url}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;display:block"/>`:'<span style="font-size:2.5rem;opacity:.35">📚</span>'}
      ...
      <div style="padding:1rem">
        ...
        <p style="color:#6b7280;font-size:.82rem;margin-bottom:.75rem;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden">${c.description||''}</p>
...
function buyModal(id,title,price,currency){
  pendingCourse={id,title,price,currency};
  document.getElementById('pm-title').textContent=title;
  document.getElementById('pm-price').textContent=fmt(price,currency);
  const balP=fmt(CP?.balance||0,CP?.currency||'XOF');
  const balR=fmt(CP?.referral_balance||0,CP?.currency||'XOF');
  document.getElementById('pm-bal-principal').textContent=balP;
  document.getElementById('pm-bal-parrainage').textContent=balR;
  // Remettre la sélection par défaut sur "principal"
  document.querySelector('input[name="pay-source"][value="principal"]').checked=true;
...
async function confirmPay(){
  ...
  if(soldeDisp<course.price){
    toast('Solde insuffisant. Veuillez recharger votre solde '+(source==='parrainage'?'parrainage':'principal')+'.','err');
    return;
  }
  ...
  try{
    ...
    const{data:pur,error}=await withTimeout(sb.from('purchases').insert({
      ...
      currency:course.currency||'XOF', status:'complete',
      payment_method:'solde_'+source
    }).select().single());
    if(error){
      const msg=error.code==='42501'?'Session expirée. Reconnectez-vous.':`Erreur commande : ${error.message}`;
      toast(msg,'err'); setBtn('pay-btn','⚡ Payer',false); return;
    }
    // Déduire le solde choisi via RPC (contourne trg_prevent_sensitive_profile_update)
    const field=source==='parrainage'?'referral_balance':'balance';
    await sbAdmin.rpc('admin_update_balance',{p_user_id:CU.id,p_amount:-(course.price),p_field:field});
    if(source==='parrainage') CP.referral_balance=(CP.referral_balance||0)-course.price;
    else CP.balance=(CP.balance||0)-course.price;
    ...
    loaded.formations=false;
    await loadBord();
    showPaySuccess(course.title);
  }catch(err){
...
function closeRecharge(){document.getElementById('modal-recharge').style.display='none';}
function goRchStep(n){[1,2,3].forEach(i=>{document.getElementById('rch-step'+i).style.display=i===n?'block':'none';});}
function doRechargeStep2(){
...
async function doRechargeConfirm(){
  const btn=document.getElementById('rch-confirm-btn');
  ...
  try{
    ...
    btn.disabled=false;btn.innerHTML='Oui, confirmer';
  }
}
...
async function refreshBalances(){
  ...
  if(btn){btn.disabled=true;btn.innerHTML='<span class="spin"></span> Actualisation…';}
  try{
    const{data:p}=await sb.from('profiles').select('balance,referral_balance,currency').eq('id',CU.id).single();
    if(p){
      if(CP)CP.balance=p.balance||0,CP.referral_balance=p.referral_balance||0;
      const balP=document.getElementById('bal-principal');
      const balR=document.getElementById('bal-parrainage');
      if(balP){balP.style.transform='scale(1.18)';balP.style.transition='transform .25s';balP.textContent=fmt(p.balance||0,p.currency||'XOF');setTimeout(()=>{balP.style.transform='scale(1)';},260);}
      if(balR){balR.style.transform='scale(1.18)';balR.style.transition='transform .25s';balR.textContent=fmt(p.referral_balance||0,p.currency||'XOF');setTimeout(()=>{balR.style.transform='scale(1)';},260);}
      if(btn){btn.innerHTML='✅ Actualisé !';btn.style.background='linear-gradient(135deg,#10b981,#059669)';}
      setTimeout(()=>{if(btn){btn.disabled=false;btn.innerHTML='🔄 Actualiser les soldes';btn.style.background='linear-gradient(135deg,#f59e0b,#d97706)';}},2000);
    }
  }catch(e){
    if(btn){btn.disabled=false;btn.innerHTML='🔄 Actualiser les soldes';}
  }
}
...
async function loadRetrait(){
  if(CP)document.getElementById('bal-display').textContent=fmt(CP.referral_balance||0,CP.currency||'XOF');
...
// ── Historique profil ─────────────────────────────────
let _histTab='achats';
async function loadHistorique(tab){
  _histTab=tab||_histTab;
  const box=document.getElementById('hist-content');
  if(!box||!CU?.id)return;
  ['achats','recharges','retraits'].forEach(t=>{
    const btn=document.getElementById('hist-tab-'+t);
    if(!btn)return;
    if(t===_histTab){btn.style.background='#2563EB';btn.style.color='white';btn.style.border='none';}
    else{btn.style.background='none';btn.style.color='#6b7280';btn.style.border='1.5px solid #e5e7eb';}
  });
  box.innerHTML='<p style="color:#9ca3af;text-align:center;padding:1.5rem">Chargement…</p>';
  try{
    if(_histTab==='achats'){
      // sbAdmin bypasse le RLS (auth.uid() = null avec Firebase) pour lire les achats
      const{data,error}=await sbAdmin.from('purchases')
        .select('amount,currency,status,purchased_at,course_id')
        .eq('user_id',CU.id).order('purchased_at',{ascending:false}).limit(20);
      if(error)throw error;
      if(!data||data.length===0){box.innerHTML='<p style="color:#9ca3af;text-align:center;padding:1.5rem">Aucun achat</p>';return;}
      // Récupérer les titres des formations séparément
      const ids=[...new Set(data.map(r=>r.course_id).filter(Boolean))];
      const{data:courses}=ids.length?await sbAdmin.from('courses').select('id,title').in('id',ids):{data:[]};
      const cmap=Object.fromEntries((courses||[]).map(c=>[c.id,c.title]));
      const smap={complete:['✅','Complété','#d1fae5','#065f46'],en_attente:['⏳','En attente','#fef3c7','#92400e'],rejete:['❌','Rejeté','#fee2e2','#991b1b']};
      box.innerHTML=data.map(r=>{
        const[ic,lb,bg,co]=smap[r.status]||smap.en_attente;
        const d=new Date(r.purchased_at).toLocaleDateString('fr-FR',{day:'numeric',month:'short',year:'numeric'});
        return`<div style="display:flex;align-items:center;justify-content:space-between;padding:.75rem;background:#f8fafc;border-radius:.75rem;margin-bottom:.5rem">
          <div style="flex:1;min-width:0"><div style="font-weight:700;font-size:.875rem;color:#1F2937;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${cmap[r.course_id]||'Formation'}</div><div style="font-size:.72rem;color:#9ca3af">${d}</div></div>
          <div style="text-align:right;flex-shrink:0;margin-left:.5rem"><div style="font-weight:800;color:#2563EB;font-size:.9rem">${fmt(r.amount,r.currency)}</div><span style="background:${bg};color:${co};font-size:.68rem;font-weight:700;padding:1px 7px;border-radius:9999px">${ic} ${lb}</span></div>
        </div>`;
      }).join('');
    } else if(_histTab==='recharges'){
      const{data,error}=await sbAdmin.from('recharge_requests')
        .select('amount,status,created_at,phone,country_code')
        .eq('user_id',CU.id).order('created_at',{ascending:false}).limit(20);
      if(error)throw error;
      if(!data||data.length===0){box.innerHTML='<p style="color:#9ca3af;text-align:center;padding:1.5rem">Aucune demande de rechargement</p>';return;}
      const smap={en_attente:['⏳','En attente','#fef3c7','#92400e'],traite:['✅','Validé','#d1fae5','#065f46'],rejete:['❌','Rejeté','#fee2e2','#991b1b']};
      box.innerHTML=data.map(r=>{
        const[ic,lb,bg,co]=smap[r.status]||smap.en_attente;
        const d=new Date(r.created_at).toLocaleDateString('fr-FR',{day:'numeric',month:'short',year:'numeric'});
        return`<div style="display:flex;align-items:center;justify-content:space-between;padding:.75rem;background:#f8fafc;border-radius:.75rem;margin-bottom:.5rem">
          <div><div style="font-weight:700;font-size:.875rem;color:#1F2937">➕ Rechargement</div><div style="font-size:.72rem;color:#9ca3af">${d} · ${r.country_code||''} ${r.phone}</div></div>
          <div style="text-align:right"><div style="font-weight:800;color:#10b981;font-size:.9rem">+${fmt(r.amount,'XOF')}</div><span style="background:${bg};color:${co};font-size:.68rem;font-weight:700;padding:1px 7px;border-radius:9999px">${ic} ${lb}</span></div>
        </div>`;
      }).join('');
    } else {
      const{data,error}=await sbAdmin.from('withdrawals')
        .select('amount,currency,status,created_at,method')
        .eq('user_id',CU.id).order('created_at',{ascending:false}).limit(20);
      if(error)throw error;
      if(!data||data.length===0){box.innerHTML='<p style="color:#9ca3af;text-align:center;padding:1.5rem">Aucun retrait</p>';return;}
      const smap={en_attente:['⏳','En attente','#fef3c7','#92400e'],en_cours:['🔄','En cours','#dbeafe','#1d4ed8'],complete:['✅','Complété','#d1fae5','#065f46'],rejete:['❌','Rejeté','#fee2e2','#991b1b']};
      const mmap={mobile_money:'📱 Mobile Money',carte_bancaire:'💳 Carte',autre:'🔄 Autre'};
      box.innerHTML=data.map(r=>{
        const[ic,lb,bg,co]=smap[r.status]||smap.en_attente;
        const d=new Date(r.created_at).toLocaleDateString('fr-FR',{day:'numeric',month:'short',year:'numeric'});
        return`<div style="display:flex;align-items:center;justify-content:space-between;padding:.75rem;background:#f8fafc;border-radius:.75rem;margin-bottom:.5rem">
          <div><div style="font-weight:700;font-size:.875rem;color:#1F2937">💸 Retrait ${mmap[r.method]||''}</div><div style="font-size:.72rem;color:#9ca3af">${d}</div></div>
          <div style="text-align:right"><div style="font-weight:800;color:#ef4444;font-size:.9rem">-${fmt(r.amount,r.currency)}</div><span style="background:${bg};color:${co};font-size:.68rem;font-weight:700;padding:1px 7px;border-radius:9999px">${ic} ${lb}</span></div>
        </div>`;
      }).join('');
    }
  }catch(e){box.innerHTML=`<p style="color:#ef4444;text-align:center;padding:1.5rem">Erreur : ${e.message}</p>`;}
}
...
async function loadRecompensesTab(tab){
  ...
  ['events','mine','points','partners'].forEach(t=>{
  ...
  if(tab==='events'){
    ...
    cont.innerHTML=(evts||[]).map(e=>{
      ...
      let joinBtn='';
      if(isPast){
        joinBtn=`<button disabled style="background:#e5e7eb;color:#9ca3af;border:none;border-radius:.75rem;padding:.625rem 1rem;font-weight:700;font-size:.8rem;cursor:not-allowed">🏁 Événement terminé</button>`;
      }else if(appStatus){
      ...
      return `<div style="background:white;border-radius:1rem;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.08);margin-bottom:1rem">
        ${e.image_url?`<div style="height:140px;overflow:hidden"><img src="${e.image_url}" style="width:100%;height:100%;object-fit:cover;display:block"/></div>`:''}
        <div style="padding:1rem">
...
function openPartnerInfo(p){
  const el=document.createElement('div');
  el.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:1.25rem';
  el.onclick=function(ev){if(ev.target===el)el.remove();};
  el.innerHTML=`<div style="background:white;border-radius:1.5rem;padding:1.75rem;max-width:380px;width:100%;box-shadow:0 25px 60px rgba(0,0,0,.3)">
    <div style="display:flex;align-items:center;gap:1rem;margin-bottom:1rem">
      ${p.logo_url?`<img src="${p.logo_url}" style="width:4rem;height:4rem;object-fit:contain;border-radius:.75rem;flex-shrink:0"/>`:'' }
      <div>
        <div style="font-weight:900;font-size:1.1rem;color:#1F2937">${p.name}</div>
        ${p.website_url?`<a href="${p.website_url}" target="_blank" style="font-size:.78rem;color:#2563eb;text-decoration:none">${p.website_url}</a>`:''}
      </div>
    </div>
    <p style="font-size:.9rem;color:#374151;line-height:1.6;margin-bottom:1.25rem">${p.description||'Aucune description.'}</p>
    <button onclick="this.closest('div[style*=fixed]').remove()" style="width:100%;padding:.75rem;background:#2563eb;color:white;border:none;border-radius:.875rem;font-weight:700;cursor:pointer">Fermer</button>
  </div>`;
  document.body.appendChild(el);
}
...
async function submitVideo(){
  ...
  // Vérifier si déjà soumise et non refusée
  const{data:existing}=await sb.from('recommendation_videos').select('id,status').eq('user_id',CU.id).neq('status','rejected').limit(1);
  if(existing&&existing.length>0){
    if(msgEl){msgEl.style.display='block';msgEl.style.background='#fef3c7';msgEl.style.color='#92400e';msgEl.textContent='⚠️ Vous avez déjà soumis une vidéo. Notre équipe est en train de l\'examiner.';}
    return;
  }
  const{error}=await sb.from('recommendation_videos').insert({user_id:CU.id,video_url:url});
  if(error){toast('Erreur. Réessayez.','err');return;}
  if(msgEl){msgEl.style.display='block';msgEl.style.background='#d1fae5';msgEl.style.color='#059669';msgEl.textContent='✅ Vidéo soumise ! Elle sera examinée sous peu. +500 pts si approuvée.';}
...
const _tutSteps = [
  ...
  {icon:'🛒', title:'Achetez une formation', desc:'Cliquez sur "Acheter" sur n\'importe quelle formation. Suivez les instructions de paiement sécurisé via PayDunya (Mobile Money, carte…).', tab:'bord'},
...
const _legalContent={
  ...
  <div class="section" id="paiement">
    ...
    <ul>
      ...
      <li>PayDunya (paiement en ligne sécurisé — Mobile Money, carte bancaire)</li>
    </ul>
